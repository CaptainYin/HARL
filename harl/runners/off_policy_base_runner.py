...
from harl.common.buffers.off_policy_buffer_ep import OffPolicyBufferEP#,OffPolicyBufferEPHer
...
class OffPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        self.use_her =False# algo_args["algo"]["use_her"]
        ...
        if self.algo_args["render"]["use_render"]:  # make envs for rendering
            ...
            self.envs = make_train_env(args["env"], algo_args["seed"]["seed"], self.rollout_threads, env_args)
        ...
        self.agent_deaths = np.zeros(
            (self.rollout_threads, self.num_agents, 1)
        )
        ...
        # print(f"share_observation_space: {self.envs.share_observation_space}, observation_space: {self.envs.observation_space}, action_space: {self.envs.action_space}")

        if self.share_param:
            self.actor = []
            self.actor.append(ALGO_REGISTRY[args["algo"]]({**algo_args["model"], **algo_args["algo"]}, self.envs.observation_space[0], self.envs.action_space[0], device=self.device))
            ...
            self.actor = []
            for agent_id in range(self.num_agents):
                self.actor.append(ALGO_REGISTRY[args["algo"]]({**algo_args["model"], **algo_args["algo"]}, self.envs.observation_space[agent_id], self.envs.action_space[agent_id], device=self.device))

        if not self.algo_args["render"]["use_render"]:
            self.critic = CRITIC_REGISTRY[args["algo"]]({**algo_args["train"], **algo_args["model"], **algo_args["algo"]}, self.envs.share_observation_space[0], self.envs.action_space, self.num_agents, self.state_type, device=self.device)
            if self.state_type == "EP":
                if self.use_her:
                    self.buffer = OffPolicyBufferEPHer({**algo_args["train"], **algo_args["model"], **algo_args["algo"]}, self.envs.share_observation_space[0], self.num_agents, self.envs.observation_space, self.envs.action_space)
                else:
                    self.buffer = OffPolicyBufferEP({**algo_args["train"], **algo_args["model"], **algo_args["algo"]}, self.envs.share_observation_space[0], self.num_agents, self.envs.observation_space, self.envs.action_space)

            elif self.state_type == "FP":
                self.buffer = OffPolicyBufferFP({**algo_args["train"], **algo_args["model"], **algo_args["algo"]}, self.envs.share_observation_space[0], self.num_agents, self.envs.observation_space, self.envs.action_space)
            else:
                raise NotImplementedError

        if self.algo_args["train"].get("use_valuenorm", False):
    ...
    def run(self):
        ...
        if self.algo_args["render"]["use_render"]:  # render, not train
            self.render()
            return
        self.train_episode_rewards = np.zeros(self.rollout_threads)
        ...
        # train and eval
        steps = self.algo_args["train"]["num_env_steps"] // self.rollout_threads
        update_num = int(  # update number per train
            self.algo_args["train"]["update_per_train"]
            * self.algo_args["train"]["train_interval"]
        )
        for step in range(1, steps + 1):
            actions = self.get_actions(obs, available_actions, add_random=True)
            new_obs, new_share_obs, rewards, dones, infos, new_available_actions = self.envs.step(actions)  # rewards: (n_threads, n_agents, 1); dones: (n_threads, n_agents)
            # available_actions: (n_threads, ) of None or (n_threads, n_agents, action_number)
            next_obs, next_share_obs, next_available_actions = new_obs.copy(), new_share_obs.copy(), new_available_actions.copy()
            data = (share_obs, obs.transpose(1, 0, 2), actions.transpose(1, 0, 2), available_actions.transpose(1, 0, 2) if len(np.array(available_actions).shape) == 3 else None, rewards, dones, infos, next_share_obs, next_obs, next_available_actions.transpose(1, 0, 2) if len(np.array(available_actions).shape) == 3 else None)
            self.insert(data)
            obs = new_obs
            ...
            if step % self.algo_args["train"]["eval_interval"] == 0:
                cur_step = self.algo_args["train"]["warmup_steps"] + step * self.rollout_threads
                if self.algo_args["eval"]["use_eval"]:
                    print(f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Evaluation at step {cur_step} / {self.algo_args['train']['num_env_steps']}:")
                    self.eval(cur_step)
                else:
                    print(f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {cur_step} / {self.algo_args['train']['num_env_steps']}, average step reward in buffer: {self.buffer.get_mean_rewards()}.\n")
                    if len(self.done_episodes_rewards) > 0:
                        aver_episode_rewards = np.mean(self.done_episodes_rewards)
                        print(f"Some episodes done, average episode reward is {aver_episode_rewards}.\n")
                        self.log_file.write( ",".join(map(str, [cur_step, aver_episode_rewards])) + "\n")
                        self.log_file.flush()
                        self.done_episodes_rewards = []
                self.save()
    ...
    def warmup(self):
        ...
        warmup_steps = self.algo_args["train"]["warmup_steps"] // self.rollout_threads
        # obs, share_obs, available_actions: (n_threads, n_agents, dim)
    ...
    def insert(self, data):
        ...
        if self.state_type == "EP":
            terms = np.full((self.rollout_threads, 1), False)
            for i in range(self.rollout_threads):
                if dones_env[i]:
                    if not infos[i][0].get("bad_transition", False):
                        terms[i][0] = True
        elif self.state_type == "FP":
            terms = np.full((self.rollout_threads, self.num_agents, 1), False)
            for i in range(self.rollout_threads):
                for agent_id in range(self.num_agents):
                    if dones[i][agent_id]:
                        if not infos[i][agent_id].get("bad_transition", False):
        ...
        for i in range(self.rollout_threads):
            if dones_env[i]:
                ...
                self.agent_deaths = np.zeros((self.rollout_threads, self.num_agents, 1))
        ...
        if self.state_type == "EP":
            if "scenario" in self.env_args and self.env_args["scenario"] == "simple_reference_sparserew_v1":
                herdata = np.concatenate([ np.expand_dims(np.vstack([infos[i][0]["her"], infos[i][1]["her"]]),axis=0) for i in range(self.rollout_threads) ])
                prev_herdata = np.concatenate([ np.expand_dims(np.vstack([infos[i][0]["prev_her"], infos[i][1]["prev_her"]]),axis=0) for i in range(self.rollout_threads) ])
                data = (
                    ...
                    next_available_actions,  # None or (n_agents, n_threads, next_action_number)
                    herdata, #(n_threads,n_agents,  her_dim)
                    prev_herdata
                )
            else:
                data = (
    ...
    def sample_actions(self, available_actions=None):
        ...
        for agent_id in range(self.num_agents):
            action = []
            for thread in range(self.rollout_threads):
        ...
        if self.args["algo"] == "hasac":
            ...
            for agent_id in range(self.num_agents):
                if (len(np.array(available_actions).shape) == 3):  # (n_threads, n_agents, action_number)
                    actions.append(_t2n(self.actor[agent_id].get_actions(obs[:, agent_id], available_actions[:, agent_id], add_random)))
                else:  # (n_threads, ) of None
                    actions.append(_t2n(self.actor[agent_id].get_actions(obs[:, agent_id], stochastic=add_random)))
        else:
            actions = []
            for agent_id in range(self.num_agents):
    ...
    def eval(self, step):
        ...
        if "football" in self.args["env"]:
            eval_score_cnt = 0
        episode_lens = []
        one_episode_len = np.zeros(self.algo_args["eval"]["n_eval_rollout_threads"], dtype=np.int32)
        ...
        while True:
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                eval_episode_rewards = np.concatenate([rewards for rewards in eval_episode_rewards if rewards])
                eval_avg_rew = np.mean(eval_episode_rewards)
                eval_avg_len = np.mean(episode_lens)
                if "smac" in self.args["env"]:
                    print("Eval win rate is {}, eval average episode rewards is {}, eval average episode length is {}.".format(
                            eval_battles_won / eval_episode, eval_avg_rew, eval_avg_len))
                elif "football" in self.args["env"]:
                    print("Eval score rate is {}, eval average episode rewards is {}, eval average episode length is {}.".format(
                ...
                if "smac" in self.args["env"]:
                    self.log_file.write(",".join(map(str, [step, eval_avg_rew, eval_avg_len, eval_battles_won / eval_episode])) + "\n")
                elif "football" in self.args["env"]:
                    self.log_file.write(",".join(map(str, [step, eval_avg_rew, eval_avg_len, eval_score_cnt / eval_episode])) + "\n")
                else:
    ...
    def restore(self):
        ...
        if not self.algo_args["render"]["use_render"]:
            ...
            if self.value_normalizer is not None:
                self.value_normalizer.load_state_dict(torch.load(f"{self.algo_args['train']['model_dir']}/value_normalizer.pt"))
