...
class BaseLogger:
    ...
    def episode_log( self, actor_train_infos, critic_train_info, actor_buffer, critic_buffer ):
        ...
        self.total_num_steps = ( self.episode * self.algo_args["train"]["episode_length"] * self.algo_args["train"]["n_rollout_threads"] )
        self.end = time.time()
        print(
            "Env {} Task {} Algo {} Exp {} updates {}/{} episodes, total num timesteps {}/{}, FPS {}, Tc {:.2f}, Tr {:.2f}.".format(
                self.args["env"],
                self.task_name,
                self.args["algo"],
                self.args["exp_name"],
                ...
                (self.algo_args["train"]["num_env_steps"]-self.total_num_steps)/(self.total_num_steps / (self.end - self.start))/3600,
            )
        )
    ...
    def eval_log(self, eval_episode):
        """Log evaluation information."""
        self.eval_episode_rewards = np.concatenate( [rewards for rewards in self.eval_episode_rewards if rewards] )
        eval_env_infos = { "eval_average_episode_rewards": self.eval_episode_rewards, "eval_max_episode_rewards": [np.max(self.eval_episode_rewards)], }
