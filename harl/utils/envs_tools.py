...
def get_shape_from_act_space(act_space):
    ...
    if act_space.__class__.__name__ == "Discrete":
        ...
        act_shape = act_space.shape[0]
    elif act_space.__class__.__name__ == "Tuple":#yinheqing 2025/8/24
        act_shape = 2+1
    return act_shape
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                ...
                env = LAGEnv(env_args)
            elif env_name == "lasercar":
                from harl.envs.lasercar.env_core import EnvCore
                if env_args['scenario']=='mix':
                    scenario_list=['CA','Cube','Warehouse','scenario1','scenario3','scenario6']#'Unmaze',
                    env_args['scenario']=scenario_list[rank%len(scenario_list)]
                env = EnvCore(env_args)
            else:
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                ...
                from harl.envs.lasercar.env_core import EnvCore
                if env_args['scenario']=='mix':
                    env_args['scenario']='Cube'
                    # env_args['goal_num']=8
                    # env_args['num_agents']=8
                    # env_args['Car_resettype']='diag'
                env_args['EnvId']=rank
                env = EnvCore(env_args)
                # env.EnvId=rank
            else:
