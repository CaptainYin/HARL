...
class ACTLayer(nn.Module):
    ...
    def __init__(
        ...
        self.tuple = False #yinheqing 2025/8/24
        ...
        elif action_space.__class__.__name__ == "Tuple": #yinheqing 2025/8/24
            self.tuple = True
            # action_dims = action_space.nvec
            action_outs = [DiagGaussian(inputs_dim, action_space[0].shape[0], initialization_method, gain, args),Categorical(inputs_dim, action_space[1].n, initialization_method, gain)]
            self.action_outs = nn.ModuleList(action_outs)
    def forward(self, x, available_actions=None, deterministic=False):
        ...
        if self.multidiscrete_action or self.tuple:
            actions = []
            action_log_probs = []
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None):
        ...
        if self.multidiscrete_action:
            ...
            action_distribution0 = self.action_outs[0](x, available_actions)
            # print(action.shape)
            action_log_probs.append(action_distribution0.log_probs(action[:,:2]))
            action_distribution1 = self.action_outs[0](x, available_actions)
            action_log_probs.append(action_distribution1.log_probs(action[:,2:]))            
            if active_masks is not None:
                dist_entropy.append((action_distribution0.entropy() * active_masks.squeeze(-1)).sum() / active_masks.sum())
                dist_entropy.append((action_distribution1.entropy() * active_masks.squeeze(-1)).sum() / active_masks.sum())
            else:
                dist_entropy.append(action_distribution0.entropy().mean())
                dist_entropy.append(action_distribution1.entropy().mean())
            action_log_probs = torch.cat(action_log_probs, dim=-1).sum(dim=-1, keepdim=True)
            dist_entropy = (torch.tensor(dist_entropy, dtype=torch.float32).sum(dim=-1, keepdim=True).mean())
            return action_log_probs, dist_entropy, None
            ...
            if active_masks is not None:
                if self.action_type == "Discrete":
                    dist_entropy = (action_distribution.entropy() * active_masks.squeeze(-1)).sum() / active_masks.sum()
                else:
                    dist_entropy = (action_distribution.entropy() * active_masks.squeeze(-1)).sum() / active_masks.sum()
            else:
